...
data=csvread("D:\Research_work\My_covid_research\dataset_preprocessing\SG_spline_SIQRD.csv");
err_record=[];
parr_record=[];
observer=0;
%SIQRD states
...
data_out=p_estim;
namer_in=join(["input_new\states_",num2str(observer),".csv"]);
namer_out=join(["output_new\parameters_",num2str(observer),".csv"]);
