...
hor=173;
curr_par=csvread("D:\Research_work\My_covid_research2\dataset_preprocessing\parameter_estim\data_augmentation\parameter_generation\parameter_generated2.csv");
a=size(curr_par);
...
for i=1:a(1)
    for j=1:hor
        if my_ind<threshold
        curr_stat=csvread(join(["D:\Research_work\My_covid_research2\dataset_preprocessing\parameter_estim\input\states_",num2str(j-1),".csv"]));
...
%         plot(days',I)
        ...
        name_output=join(["D:\Research_work\My_covid_research2\dataset_preprocessing\parameter_estim\output\parameters_",num2str(offsetter+1+my_ind),".csv"]);
        name_input=join(["D:\Research_work\My_covid_research2\dataset_preprocessing\parameter_estim\input\states_",num2str(offsetter+1+my_ind),".csv"]);
